...
def generate_report(yearly_totals: dict,
    ...
    for year in sorted(yearly_totals.keys()):
        ...
        if business_plan_repo:
            report_lines.append("-" * 90)
            report_lines.append(
                f"{'Envelope':<20} {'Spent':>15} {'Planned':>15} {'Diff':>15} {'Added':>15}"
            )
        else:
            report_lines.append("-" * 60)
            report_lines.append(
                f"{'Envelope':<20} {'Spent':>15} {'Added':>15}"
            )
        report_lines.append("-" * 90 if business_plan_repo else "-" * 60)
        ...
        for env_id in yearly_totals[year]:
            ...
            if business_plan_repo:
                try:
                    planned = business_plan_repo.get_planned_for_envelope(env_name, year)
                except:
                    planned = 0.0

                diff = planned - totals['spent']
                year_lines.append(
                    f"{env_name:<20} {totals['spent']:>15.2f} {planned:>15.2f} {diff:>15.2f} {totals['added']:>15.2f}"
                )
            else:
                year_lines.append(
                    f"{env_name:<20} {totals['spent']:>15.2f} {totals['added']:>15.2f}"
                )
        ...
        if business_plan_repo:
            report_lines.append(
                f"{'TOTAL':<20} {year_total_spent:>15.2f} {'':>15} {'':>15} {year_total_added:>15.2f}"
            )
        else:
            report_lines.append(
                f"{'TOTAL':<20} {year_total_spent:>15.2f} {year_total_added:>15.2f}"
            )
