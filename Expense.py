...
class Expense:
    ...
    def toSaveLine(self):
        return u"{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}".format(self.__id, self.__date, self.__value, self.__desc, self.__fromId, self.__toId, self.__line, self.__manual)

    @classmethod
    def fromXml(cls, el):
        parts = []
        parts.append(uuid.UUID(el.get("id")))
        parts.append(datetime.datetime(*map(int, re.split('[^\d]', el.get("date"))[:-1])))
        parts.append(float(el.get("value")))
        parts.append(el.get("desc"))
        parts.append(int(el.get("fromId")))
        parts.append(int(el.get("toId")))
        parts.append(el.get("line"))
        parts.append(bool(el.get("manual")))
